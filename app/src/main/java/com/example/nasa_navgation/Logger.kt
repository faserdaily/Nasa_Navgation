...
object Logger {
     ...
     * @param Tonoff Toast 開關（true = 開啟，false = 關閉，預設為 false）
     * @param tag Logcat 標籤（預設為 Constants.Logger.DEFAULT_TAG）
     */
    fun log(
        message: Any?,
        context: Context? = null,
        Lonoff: Boolean = true,
        Tonoff: Boolean = false,
        tag: String = Constants.Logger.DEFAULT_TAG
    ) {
        // 將訊息轉換為字串（null 會顯示為 Constants.Logger.NULL_STRING）
        val messageStr = message?.toString() ?: Constants.Logger.NULL_STRING
        ...
        if (Tonoff) {
            ...
            if (toastContext != null) {
                ...
                Log.w(tag, Constants.Logger.TOAST_CONTEXT_NULL)
            }
        }
    }
}
